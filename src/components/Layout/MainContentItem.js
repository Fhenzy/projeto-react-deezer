...
function MainContentItem({item, index}) {
    ...
    const prefAddButton = item.type === 'track' ? <button onClick={() => { addPreferedMusicService(item);  }}>Adicionar aos Favoritos</button> : null;
    const prefDelButton = item.type === 'track' ? <button onClick={() => { removePreferedMusicService(index); }}>Remover dos Favoritos</button> : null;
